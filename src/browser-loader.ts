...
function installEncryptedFetch(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const isPublicGet = request.method === "GET" && url.origin === location.origin && PUBLIC_JSON_PATHS.has(url.pathname);
    ...
    try {
      ...
      console.error("[马会资料] 接口解密失败", error);
      throw error;
    }
  };
}
...
async function mount(): Promise<void> {
  try {
    ...
    console.error("[马会资料] 页面自动解密失败", error);
    showFriendlyError();
  }
}
